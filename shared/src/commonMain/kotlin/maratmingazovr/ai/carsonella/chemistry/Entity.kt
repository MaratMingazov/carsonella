...
//    H2;
    ...
    // Валентность — сколько ковалентных связей атом образует, чтобы достроить внешнюю оболочку
    // (октет/дублет). ВЫЧИСЛЯЕТСЯ из Z, не таблицей: заполняем оболочки [2, 8, 8] (период 1–3, где
    // октет/дублет валиден), берём внешнюю V → полная оболочка = 0 (благородный газ), V ≤ 4 → V,
    // иначе 8 − V. Тяжелее аргона (Z > 18) → 0: октет там не работает (переменная валентность,
    // d-орбитали), ковалентно такие пока не связываем. Так «тяжёлые не связываются» инкапсулировано
    // здесь — правилу образования связи отдельный гард по Z не нужен, достаточно valence() > 0.
    fun valence(): Int {
...
data class Details(
    ...
    // Реальные атомные радиусы в пикометрах (пм) — язык справочных таблиц (Cordero 2008 / Bondi 1964),
    // копируются 1:1. Зависят только от Z → у изотопов одного элемента одинаковы (как p). nullable:
    // у не-атомов (фотон/электрон/звезда) радиуса-атома нет. Пока чистые ДАННЫЕ: к физике/рендеру не подключены.
