...
data class MoleculeGraph(
    ...
    /**
     * Энергетическая лестница молекулы — в той же форме, что атомная [Element.energyLevels]: список
     * уровней, где ПОСЛЕДНИЙ = порог (первый потенциал ионизации, IP). Кэш на графе (иммутабелен).
     *
     * Сегодня лестница содержит РОВНО ОДИН уровень — порог ионизации; рунгов электронного возбуждения
     * у молекулы пока нет. Почему не «атомные рунги, склеенные вместе»: электронно-возбуждённые
     * состояния молекулы рождаются из её молекулярных орбиталей, а не из атомных спектральных линий
     * (спектр H₂ ≠ спектр двух атомов H). Собрать лестницу из атомных = правдоподобная ложь; честно —
     * один реально определённый порог. Настоящие рунги возбуждения потребуют спектроскопической
     * таблицы по молекулам (отдельный шаг). Форма List — чтобы молекула и атом жили по одному API
     * (`.last()` = IP у обоих) и, когда рунги появятся, здесь ничего переделывать не пришлось.
     *
     * Порог — минимальный первый IP среди атомов: электрон уходит с атома, где его легче всего
     * оторвать. Берётся из атомной лестницы [Element.energyLevels] (нейтральный атом: electrons = Z =
     * protons; последний уровень = первый IP). Приближение: реальный IP молекулы определяется её МО, но
     * эмёрджентный «минимум по атомам» ловит порядок величины без таблицы IP по молекулам — в духе
     * [Element.valence]/[BondEnergy].
     *
