 ...
 * «это та же молекула?», а `merge` перенумеровывает узлы, после чего сохранённые координаты протухли бы.
 *
 * Живёт в `shared`, а не в рендере, потому что нужна ОБОИМ: рендер по ней рисует, а правилам реакций
 * она нужна, чтобы выбирать узел по геометрии («ближайший к партнёру»), а не по номеру — так растёт
 * молекула (MoleculeGrowth). Единицы — мировые, они же пиксельные: перевод
