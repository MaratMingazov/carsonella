...
class MoleculeGrowthTest {
    ...
    private fun hydroxyl(x: Float): Molecule {
        ...
        return Molecule(nextId++, Species.Molecular(graph), Position(x, 0f), Vec2D(0f, 0f), 0f, 0f, electrons = 9)
            .also { it.setEnvironment(env) }
    }
    ...
    private fun water(x: Float): Molecule {
        ...
        return Molecule(nextId++, Species.Molecular(graph), Position(x, 0f), Vec2D(0f, 0f), 0f, 0f, electrons = 10)
            .also { it.setEnvironment(env) }
    }
    ...
    fun hydroxylPlusHydrogenMakesWater() {
        ...
        assertFalse(graph.hasFreeValence)                  // вода насыщена
