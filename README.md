...
- `App.kt` — корень: переключение экранов (меню, игра, карта, язык, «об игре») и `PlayerState` — настройки (язык) отдельно от прогресса (открытые каноны, пройденные цели). Живёт выше `GameScreen`, поэтому мир умирает при выходе в меню, а открытия нет. На диск пока не пишется.
