...
sealed interface Goal {
    data class Molecule(val id: MoleculeId) : Goal
...
val Goal.known: KnownMolecule? get() = (this as? Goal.Molecule)?.let { MoleculeRegistry.byId(it.id) }
...
val LEVELS = listOf(
    ...
    Level(LevelId.DIHYDROGEN, requires = setOf(LevelId.HYDROGEN_ATOM), goal = Goal.Molecule(MoleculeId.DIHYDROGEN), inventory = mapOf(atom(Element.HYDROGEN) to 2),
        task = Prose(
    ...
    Level(LevelId.HYDROXYL, requires = setOf(LevelId.DIHYDROGEN), goal = Goal.Molecule(MoleculeId.HYDROXYL), inventory = mapOf(atom(Element.HYDROGEN) to 1, atom(Element.OXYGEN_16) to 1),
        task = Prose(
    ...
    Level(LevelId.WATER, requires = setOf(LevelId.HYDROXYL), goal = Goal.Molecule(MoleculeId.WATER), inventory = mapOf(atom(Element.HYDROGEN) to 2, atom(Element.OXYGEN_16) to 1),
        task = Prose(
    ...
    Level(LevelId.DIOXYGEN, requires = setOf(LevelId.WATER), goal = Goal.Molecule(MoleculeId.DIOXYGEN), inventory = mapOf(atom(Element.OXYGEN_16) to 2),
        task = Prose(
    ...
    Level(LevelId.PEROXIDE, requires = setOf(LevelId.DIOXYGEN), goal = Goal.Molecule(MoleculeId.HYDROGEN_PEROXIDE), inventory = mapOf(atom(Element.HYDROGEN) to 2, atom(Element.OXYGEN_16) to 2),
        task = Prose(
            ru = "Теперь давай попробуем построить перекись водорода",
            en = "Now let's try building hydrogen peroxide",
        )),
    Level(LevelId.PEROXIDE_SPLIT, requires = setOf(LevelId.PEROXIDE), goal = Goal.Molecule(MoleculeId.HYDROXYL), inventory = mapOf(known(MoleculeId.HYDROGEN_PEROXIDE) to 1, atom(Element.PHOTON) to 3),
        task = Prose(
