...
sealed interface LevelGoal {
    data class Molecule(val id: KnownMoleculeId) : LevelGoal
    data class Atom(val element: Element, val electrons: Int = neutralElectrons(element)) : LevelGoal
}

/** Запись реестра для молекулярной цели: у неё есть имя, картинка и факт. У атомарной — нет. */
val LevelGoal.known: KnownMoleculeDetails? get() = (this as? LevelGoal.Molecule)?.id?.details
...
val LEVELS = listOf(
    Level(LevelId.HYDROGEN_ATOM, requires = emptySet(), levelGoal = LevelGoal.Atom(Element.HYDROGEN), inventory = mapOf(proton to 3, electron to 3),
    ...
    Level(LevelId.DIHYDROGEN, requires = setOf(LevelId.HYDROGEN_ATOM), levelGoal = LevelGoal.Molecule(KnownMoleculeId.DIHYDROGEN), inventory = mapOf(atom(Element.HYDROGEN) to 2),
        taskDescription = TranslatedText(
    ...
    Level(LevelId.HYDROXYL, requires = setOf(LevelId.DIHYDROGEN), levelGoal = LevelGoal.Molecule(KnownMoleculeId.HYDROXYL), inventory = mapOf(atom(Element.HYDROGEN) to 1, atom(Element.OXYGEN_16) to 1),
        taskDescription = TranslatedText(
    ...
    Level(LevelId.WATER, requires = setOf(LevelId.HYDROXYL), levelGoal = LevelGoal.Molecule(KnownMoleculeId.WATER), inventory = mapOf(atom(Element.HYDROGEN) to 2, atom(Element.OXYGEN_16) to 1),
        taskDescription = TranslatedText(
    ...
    Level(LevelId.DIOXYGEN, requires = setOf(LevelId.WATER), levelGoal = LevelGoal.Molecule(KnownMoleculeId.DIOXYGEN), inventory = mapOf(atom(Element.OXYGEN_16) to 2),
        taskDescription = TranslatedText(
    ...
    Level(LevelId.PEROXIDE, requires = setOf(LevelId.DIOXYGEN), levelGoal = LevelGoal.Molecule(KnownMoleculeId.HYDROGEN_PEROXIDE), inventory = mapOf(atom(Element.HYDROGEN) to 2, atom(Element.OXYGEN_16) to 2),
        taskDescription = TranslatedText(
            ru = "Теперь давай попробуем построить перекись водорода",
            en = "Now let's try building hydrogen peroxide",
        )),
    Level(LevelId.PEROXIDE_SPLIT, requires = setOf(LevelId.PEROXIDE), levelGoal = LevelGoal.Molecule(KnownMoleculeId.HYDROXYL), inventory = mapOf(known(KnownMoleculeId.HYDROGEN_PEROXIDE) to 1, atom(Element.PHOTON) to 3),
        taskDescription = TranslatedText(
